import { getDistricts, getActiveSeason, getRankings } from '@/server/queries/player.queries';
...
export async function loadPublicPageData(options?: {
  ...
  try {
    ...
    return {
      ok: false,
      message:
        'Unable to reach the game database. On Vercel, set DATABASE_URL and AUTH_SECRET, then run migrations and seed the database.',
    };
  }
}
