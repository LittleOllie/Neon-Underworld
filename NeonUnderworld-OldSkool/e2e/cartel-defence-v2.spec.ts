...
test.describe('Faction Defence v2 — multi-player', () => {
  ...
  test('leader sees rides, transport capacity, and response force on HQ', async ({ browser }) => {
    ...
    const forces = leader.locator('section[aria-label="Faction forces"]');
    await expect(forces.getByText('Rides', { exact: true })).toBeVisible();
    await expect(forces.getByText(/Transport capacity/i)).toBeVisible();
    await expect(forces.getByText(/5 rides · 25 enforcers/i)).toBeVisible();

    const response = leader.locator('section[aria-label="Faction response"]');
    await expect(response.getByText(/Your max faction response/i)).toBeVisible();
    await expect(response.getByText(/25 enforcers/i)).toBeVisible();
  });

  test('leader can purchase faction rides from armoury', async ({ browser }) => {
    const leader = await browser.newPage();
    await loginAs(leader, PVP_BUYER_EMAIL, PVP_BUYER_PASSWORD);
    await gotoGame(leader, '/cartels');

    const armoury = leader.locator('section[aria-label="Faction armoury purchases"]');
    ...
    const forces = leader.locator('section[aria-label="Faction forces"]');
    await expect(forces.getByText(/6 rides · 30 enforcers/i)).toBeVisible({ timeout: 15_000 });
  });

  test('attacker strike deploys capped faction response and updates shared pool', async () => {
  ...
  test('member sees personal response force cap on HQ', async ({ browser }) => {
    ...
    const response = member.locator('section[aria-label="Faction response"]');
    await expect(response.getByText(/Your max faction response/i)).toBeVisible();
    const maxResponseText =
      (await response.locator('.g-row').filter({ hasText: 'Your max faction response' }).textContent()) ??
      '';
